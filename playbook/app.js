import {blankBook,validateBook,emptyPlay,CARD_NAMES,COLORS,contrastColor,swapPlays} from './model.js';
...
function makeSlot(p,i,editable=true){
  const el=document.createElement(editable?'button':'div');el.className='play-slot'+(p.overlay?' overlay':'')+(editable&&i===selected?' selected':'');
  const diagram=document.createElement('div');diagram.className='diagram';
  if(p.image){const img=document.createElement('img');img.src=p.image.data;img.alt=p.name||`Play ${p.number}`;img.draggable=false;diagram.append(img);}else{const plus=document.createElement('span');plus.className='empty-diagram';plus.textContent='＋';diagram.append(plus);}
  const banner=document.createElement('div');banner.className='play-banner';banner.style.background=p.color;banner.style.color=contrastColor(p.color);const num=document.createElement('b');num.textContent=p.number;const words=document.createElement('span');words.textContent=p.name;banner.append(num,words);el.append(diagram,banner);
  if(editable){el.type='button';el.draggable=true;el.setAttribute('aria-label',`${CARD_NAMES[Math.floor(i/8)]}, slot ${i+1}: ${p.number} ${p.name||'empty play'}`);el.setAttribute('aria-pressed',String(i===selected));el.onclick=()=>{selected=i;renderCards();renderEditor();};el.ondblclick=()=>{selected=i;renderEditor();$('replaceInput').click();};el.ondragstart=e=>{dragFrom=i;e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',String(i));};el.ondragend=()=>{dragFrom=null;document.querySelectorAll('.drag-over').forEach(x=>x.classList.remove('drag-over'));};el.ondragover=e=>{e.preventDefault();el.classList.add('drag-over');};el.ondragleave=()=>el.classList.remove('drag-over');el.ondrop=e=>{e.preventDefault();el.classList.remove('drag-over');if(e.dataTransfer.files.length){selected=i;renderEditor();uploadFiles([...e.dataTransfer.files],true).catch(report);return;}if(dragFrom!==null&&dragFrom!==i){remember();swapPlays(book,dragFrom,i);selected=i;changed();renderEditor();}dragFrom=null;};}
  return el;
}
...
function renderEditor(){const p=book.plays[selected];$('selectedPosition').textContent=`${CARD_NAMES[Math.floor(selected/8)].toUpperCase()} · SLOT ${selected+1}`;$('playNumber').value=p.number;$('playName').value=p.name;$('overlay').checked=p.overlay;$('replaceImage').textContent=p.image?'Replace play image':'Upload image for this play';$('moveTo').value=selected;$('swatches').querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.color===p.color)));renderPreview();}
...
async function uploadFiles(files,replace=false){if(busy)return;const target=selected;const slots=replace?[target]:book.plays.map((p,i)=>p.image?null:i).filter(i=>i!==null);if(!slots.length){notice('All 24 slots have images. Select a play and choose “Replace play image”.');return;}if(files.length>slots.length){notice(`There are ${slots.length} available slots. Select ${slots.length} or fewer images.`);return;}
  ...
  lock(false);if(loaded.length){remember();for(const item of loaded){const p=book.plays[item.slot];p.image=item.image;if(!p.name)p.name=item.name;}selected=loaded[0].slot;changed();renderEditor();}else status(dirty?'Unsaved changes':key?'Cloud connected':'On this device');if(errors.length)notice(errors.join(' '));
}
...
for(const [id,prop] of [['playNumber','number'],['playName','name']]){$(id).addEventListener('focus',remember);$(id).oninput=e=>{book.plays[selected][prop]=e.target.value;changed();};}
...
$('clearPlay').onclick=()=>{remember();book.plays[selected]=emptyPlay(selected);changed();renderEditor();};$('undo').onclick=()=>{if(!history.length)return;book=JSON.parse(history.pop());changed();renderAll();};
