...
begin
 ...
 b:=jsonb_set(jsonb_set(jsonb_set(b,'{plays,0,line2}','"Jet sweep"'),'{plays,0,imageMode}','"canva-326"'),'{plays,0,textSize}','5.5');
 perform public.vikings_save(repeat('a',64),'11111111-1111-4111-8111-111111111111',b,1);
 result:=public.vikings_load(repeat('a',64),'11111111-1111-4111-8111-111111111111');
 if result->'payload'<>b then raise exception 'Canva, second-line and text-size round trip failed'; end if;
 ...
 denied:=false;begin perform public.vikings_save(repeat('a',64),'11111111-1111-4111-8111-111111111111',jsonb_set(b,'{plays,0,textSize}','"5.5"'),2);exception when sqlstate 'PT400' then denied:=true;end;if not denied then raise exception 'string text size accepted';end if;
 denied:=false;begin perform public.vikings_save(repeat('a',64),'11111111-1111-4111-8111-111111111111',jsonb_set(b,'{plays,0,textSize}','5.25'),2);exception when sqlstate 'PT400' then denied:=true;end;if not denied then raise exception 'invalid text size step accepted';end if;
 denied:=false;begin perform public.vikings_save(repeat('a',64),'11111111-1111-4111-8111-111111111111',jsonb_set(b,'{plays,0,textSize}','13'),2);exception when sqlstate 'PT400' then denied:=true;end;if not denied then raise exception 'out of range text size accepted';end if;
 perform public.vikings_save(repeat('a',64),'11111111-1111-4111-8111-111111111111',jsonb_set(b,'{plays,0,textSize}','null'),2);
 result:=public.vikings_load(repeat('a',64),'11111111-1111-4111-8111-111111111111');
 if result->'payload'->'plays'->0->'textSize' is distinct from 'null'::jsonb then raise exception 'Auto text size round trip failed';end if;
