...
- Edit each play's number, two banner text lines, and banner color. The printed wristband banner is 5.636 mm tall (20% of each cell). With line 2 filled, each text field stays on its own line; shorter labels print more clearly. Leaving line 2 empty retains the original automatic wrapping. Choose Auto fit or a per-play banner text size from 4.5–12 points; manual sizes apply to both word lines and scale up on the coach sheet. The preview and PDFs share measured text layout. Oversized text is flagged in the editor and must be reduced or shortened before PDF export. Text sizes save to the cloud and backups; older playbooks default to Auto fit. Each number and banner text input has its own full-width row.
- Download a one-page Letter wristband PDF: each card's cut rectangle is 112.72 × 56.36 mm (about 4.438 × 2.219 inches), matching the existing Excel inserts whose play blocks measure approximately 28.18 mm square. The paper extends slightly under the edges of the 111 × 56 mm visible window. Each of its eight cells is 28.18 × 28.18 mm; diagrams keep their existing proportions and crop, manual wristband text sizes remain exact, and coach text retains its previous enlargement. Print at Actual size / 100%; verify both 1-inch calibration marks with a ruler.
