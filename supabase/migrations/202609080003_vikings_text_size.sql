...
begin
 ...
 for p in select value from jsonb_array_elements(p_payload->'plays') loop
  ...
  -- Optional for old backups and clients; new clients supply these fields.
  ...
  if (p ? 'textSize') and p->'textSize'<>'null'::jsonb then
   if jsonb_typeof(p->'textSize') is distinct from 'number' then raise sqlstate 'PT400' using message='Invalid banner text size.'; end if;
   if (p->>'textSize')::numeric not between 4.5 and 12 or mod((p->>'textSize')::numeric,0.5)<>0 then raise sqlstate 'PT400' using message='Invalid banner text size.'; end if;
  end if;
